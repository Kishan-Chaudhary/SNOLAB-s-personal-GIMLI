...
int main(int argc,char** argv) {
  ...
  UI->ApplyCommand("/run/numberOfThreads 8");
 ...
 // UI->ApplyCommand("/run/verbose 2");          // Set run manager verbosity
  ...
  //UI->ApplyCommand("/vis/filtering/trajectories/particleFilter-0/add neutron");
  ...
  //Wendt Fission Model, for more details
  //UI->ApplyCommand("/process/had/particle_hp/use_Wendt_fission_model true");
  UI->ApplyCommand("/control/execute Gamma_mono.mac");
  /*ui->SessionStart();
  delete ui;
  */
