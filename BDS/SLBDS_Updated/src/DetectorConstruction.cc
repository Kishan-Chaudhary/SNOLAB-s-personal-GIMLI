...
G4VPhysicalVolume *DetectorConstruction::Construct()
{
  ...
  G4Material* lead = nist->FindOrBuildMaterial("G4_Pb"); //Used nist for Pb.
  ...
  Air->AddElement(O, 28.5 * perCent);
  Air->AddMaterial(H2O, 1.5*perCent);
 worldMaterial = Air;
  ...
  G4ThreeVector PbCastle(0, 0, innerPbBasetoChamber);
  ...
  logicChamberLid = new G4LogicalVolume(solidChamberLid,            //shape
		 ...
		 new G4PVPlacement(0,                        //Placing Chamber around detectors
             ...
             //Subtract the inner volume from the outer volume, to create a castle with thickness
             G4SubtractionSolid* VLeadCastle = new G4SubtractionSolid("Lead Castle Solid", outerLeadCastleBox, innerLeadCastleBox);
         
       
             // Define logical volumes
             G4LogicalVolume* outerLeadWall = new G4LogicalVolume(VLeadCastle, lead, "Pb Castle");

             
             // Position the Pb castle
           new G4PVPlacement(0,                         // No rotation
                 PbCastle,  // Position at origin
                 outerLeadWall,           // Outer logical volume
                 "Pb castle",       // Name
                 logicWorld,              // Mother volume
                 false,                   // No boolean operation
                 0,                       // No copies
