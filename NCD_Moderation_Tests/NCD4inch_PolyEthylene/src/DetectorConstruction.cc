...
G4VPhysicalVolume* DetectorConstruction::Construct() {
 ...
 //   HumidAir->AddMaterial(H2O, massFractionWater);              // Water vapor 
  ...
  /*  G4Tubs* solidWorld = new G4Tubs("World", 0, worldRadius, worldSize / 2, 0., 360. * deg);
	...
	G4Box* solidWorld =
		new G4Box("solidWorld", 200 * cm, 200 * cm, 450 * cm); //height, widht,length for room
...
// Placing the second NCD in the opposite corner, using lHe3CuTube from previous NCD
 ...
 /*   G4Tubs* solidFrontSteelCap2 = new G4Tubs("FrontSteelCap2", 0, fHe3TubeRadius + fHe3TubeThickness, fSteelCapThickness / 2., 0., 360. * deg);
  ...
  /*  G4Tubs* solidFrontSteelCap3 = new G4Tubs("FrontSteelCap3", 0, fHe3TubeRadius + fHe3TubeThickness, fSteelCapThickness / 2., 0., 360. * deg);
   ...
   /* G4Tubs* solidAnodeWire3 = new G4Tubs("AnodeWire3", 0, fHe3AnodeDiameter / 2., anodeWireLength / 2., 0., 360. * deg);
	...
	//Polyethylene Castle Geometry
    G4int innerPolyEthyleneBase = (fHe3TubeL / 2) + 3 * cm;
    G4int innerPolyEthyleneHeight = He3NickelOR + 3 * cm;

	G4int outerPolyEthyleneBase = innerPolyEthyleneBase + 10.16 * cm; //7.62 cm = 3 inches
    G4int outerPolyEthyleneHeight = innerPolyEthyleneHeight + 10.16 * cm;

    G4Box* innerPolyEthyleneBox = new G4Box("Inner PolyEthylene Box", innerPolyEthyleneHeight, innerPolyEthyleneHeight, innerPolyEthyleneBase);
    G4Box* outerPolyEthyleneBox = new G4Box("Outer PolyEthylene Box", outerPolyEthyleneHeight, outerPolyEthyleneHeight, outerPolyEthyleneBase);

    //Subtract the inner volume from the outer volume, to create a castle with thickness with the differenc of their size
    G4SubtractionSolid* PolyEthyleneSolid = new G4SubtractionSolid("PolyEthylene Solid", outerPolyEthyleneBox, innerPolyEthyleneBox);

    // Define PolyEthylene logical volumes
    G4LogicalVolume*  PolyEthyleneCastle = new G4LogicalVolume(PolyEthyleneSolid, PolyEthylene, "PolyEthylene");

    G4ThreeVector Center(0, 0, 0);
    // Position the PolyEthylene castle
    new G4PVPlacement(0,                         // No rotation
        G4ThreeVector(shiftSide, -shiftSide, 0),  // Position at origin (0?)
        PolyEthyleneCastle,           // Outer logical volume
        "PolyEthylene castle",       // Name
        logicWorld,              // Mother volume
        false,                   // No boolean operation
        0,                       // No copies
        true);

    G4cout << "PolyEthylene Castle Created" << G4endl;


	//Water Castle Geometry
	G4int innerWaterCastleBase = outerPolyEthyleneBase ;    // (fHe3TubeL/2) + 7.62 * cm + 1 * cm
	G4int innerWaterCastleHeight = outerPolyEthyleneHeight; // He3NickelOR  + 7.62 * cm + 1 * cm

	G4int outerWaterCastleBase = innerWaterCastleBase + 30.48 * cm;
	G4int outerWaterCastleHeight = innerWaterCastleHeight + 30.48 * cm;

	// Define the inner and outer box for water castle
    G4Box* innerWaterBox = new G4Box("Inner Water Castle Box", innerWaterCastleHeight, innerWaterCastleHeight, innerWaterCastleBase);
    G4Box* outerWaterBox = new G4Box("Outer Water Castle Box", outerWaterCastleHeight, outerWaterCastleHeight, outerWaterCastleBase);

    //Subtract the inner volume from the outer volume, to create a castle with thickness with the differenc of their size
    G4SubtractionSolid* WaterCastleSolid = new G4SubtractionSolid("Water Castle Solid", outerWaterBox, innerWaterBox);

    // Define H2O logical volumes
    G4LogicalVolume* WaterCastle = new G4LogicalVolume(WaterCastleSolid, H2O, "Water Castle");

    // Position the H2O castle
 //new G4PVPlacement(0,                         // No rotation
 //    G4ThreeVector(shiftSide, -shiftSide, 0),  // Position at origin (0?)
 //       WaterCastle,           // Outer logical volume
 //       "H2O castle",       // Name
 //       logicWorld,              // Mother volume
 //       false,                   // No boolean operation
 //       0,                       // No copies
 //       true);
 //
 //G4cout << "Water Castle Created" << G4endl;
...
void DetectorConstruction::ConstructSDandField()
{
    ...
    lHe3CuTube->SetSensitiveDetector(sensDet);

    G4cout << "Sensitive Detector Set" << G4endl;
